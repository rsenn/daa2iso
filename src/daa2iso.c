...
unsigned int
daa2iso_read_bits(unsigned int bits, unsigned char* in, unsigned int in_bits, unsigned int lame, int lame_increase);
...
static void
SzFree(void* p, void* address) {
  free(address);
}
static ISzAlloc g_Alloc = {SzAlloc, SzFree};

int multi = 0, multinum, endian, daagbi = TYPE_DAA, swapped_btype[3] = {0, 1, 2};
char* multi_filename;
...
main(int argc, char* argv[]) {
  ...
  FILE *fdi, *fdo;
  u64 tot;
  u32 daa_type, i, len, insz, outsz, pwdtype, pwdcrc, daacrc,
      daas,     // amount of elements in the index table
      daas_mem, // size of the memory containing the index table
      last_chunk, ver110_x, ver110_y, daa_dataz;
  int ztype,   // for 110
      bitpos,  // for 110
      bitsize, // for 110
      bittype, // for 110
      dolame, dolzma, ver110_btype, dolamebits, lzma_filter;
  u8 ans[66], // password goes from 1 to 64 chars
      pwdkey[128], daakey[128], *filei, *fileo, *in, *out, *p;
  ...
  if(daa.version == 0x100) {
    ...
    if(ver110_x & 0x10000000) {
      ...
      // esi[8948] = 1;   // seems that if it's 1 the readbit function is not used and at its place is returned an
      // error... senseless
    }
    if(ver110_y & 0x20000) {
  ...
  for(i = 0; i < daas; i++) {
    ...
    if(daa.version == 0x100) {
      ...
      bitpos += bitsize;
      len += LZMA_PROPS_SIZE;
      ztype = daa2iso_read_bits(bittype, (u8*)daa_data, bitpos, dolamebits, 1);
      bitpos += bittype;
...
daa2iso_read_bits(unsigned int bits, unsigned char* in, unsigned int in_bits, unsigned int lame, int lame_increase) {
  ...
  unsigned int seek_bits, rem, seek = 0, ret = 0, mask = 0xffffffff;
...
void
gburner_lame(u8* data, int size, u8 crc8) {
  int i;
  u8 d;
...
void
poweriso_lame(u8* data, int size, u64 isosize) {
  int i;
  u8 a, c;
...
void
poweriso_is_shit(u8* chunk, int chunksize) {
  u32 num, bp, e10, e20, e28;
  int i;
  u8 shit[8] = {0, 1, 2, 2, 3, 3, 3, 3}, shiz[8] = {1, 1, 1, 0, 1, 0, 0, 0}, tmp;
...
get_file(void) {
  ...
  static const char filter[] = "DAA/GBI file\0"
                               "*.daa;*.gbi\0"
                               "(*.*)\0"
                               "*.*\0"
                               "\0"
...
char*
put_file(char* suggested) {
  OPENFILENAME ofn;
  char* p;
  static char filename[4096 + 10];
  static const char filter[] = "ISO file\0"
                               "*.iso\0"
                               "(*.*)\0"
                               "*.*\0"
                               "\0"
...
void
myalloc(u8** data, unsigned wantsize, unsigned* currsize) {
...
void
myfr(FILE* fd, void* data, unsigned size) {
  int len;
...
void
myfw(FILE* fd, void* data, unsigned size) {
...
int
unlzma(CLzmaDec* lzma, u8* in, u32 insz, u8* out, u32 outsz) {
  ELzmaStatus status;
  SizeT inlen, outlen;
...
int
unzip(u8* in, u32 insz, u8* out, u32 outsz) {
  tinf_init();
...
l2n_32(u32* num) {
  ...
  tmp = *num;
  *num =
      ((tmp & 0xff000000) >> 24) | ((tmp & 0x00ff0000) >> 8) | ((tmp & 0x0000ff00) << 8) | ((tmp & 0x000000ff) << 24);
}
...
int
fgetz(u8* data, int size, FILE* fd) {
  u8* p;
